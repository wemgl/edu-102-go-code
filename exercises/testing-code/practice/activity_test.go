...
func TestSuccessfulTranslateActivityGoodbyeLatvian(t *testing.T) {
	...
	var result TranslationActivityOutput
	assert.NoError(t, future.Get(&result))
	assert.Equal(t, "Ardievu", result.Translation)
}
...
func TestFailedTranslateActivityBadLanguageCode(t *testing.T) {
	...
	// Asser that the error has the expected message, which identifies
